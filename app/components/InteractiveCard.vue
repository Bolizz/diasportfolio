  ...
  >
    ...
    <header class="p-6">
      <div class="flex items-start justify-between">
        ...
        <div
          class="ml-4 inline-flex h-9 w-9 items-center justify-center rounded-xl bg-primary-600/20 text-primary-600 transition-transform duration-300 group-hover:scale-110"
        >
          ↗
        </div>
      </div>
...
const HOVER_IN_DELAY = 60; // ms (optional)
const HOVER_OUT_DELAY = 130; // ms — gives a softer feel on unhover
...
function leave(el: Element) {
  const elmt = el as HTMLElement;
  // From auto -> fixed height, then to 0
  elmt.style.height = elmt.scrollHeight + "px";
  elmt.style.opacity = "1";
  void elmt.offsetHeight;
  elmt.style.transition =
